...
<div class="myform">
  <div class="flexbox">
    <div class="search">
      <h2><b>Enter the ID of the completed request</b></h2>
      <div>
  ...
  <?php
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
      ...
      $id = $_POST["id"];
      // Store the id in session so we can use it in another php page (specificlly Report.php)
      $_SESSION['id'] = $id ;
        ...
        //getting the email of the assigned user, through the session.
        $Email = "";
        ...
        $query = "SELECT * FROM requests WHERE ID = '$id' AND Email = '$Email' AND status1 ='Completed'";
        ...
        if ($result) {
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo '<br>';
                    echo '<button type="submit">Print Report</button>';
                    echo '</form>';
                }
            } else {
              //Its not allowed for users to print the request report until its completed (Transport Request was fully successful)
              echo "<script> alert('The ID you have entered belongs to an incompleted request, please try again');</script>";
              echo "<script>window.location.href = 'genReport.php';</script>";
            }
